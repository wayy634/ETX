...
@property (nonatomic, retain) NSMutableDictionary *_accessibilityElements;
...
@synthesize _accessibilityElements = __accessibilityElements;
@synthesize  touchShadowBehind;
/////////////////////////////////////////////////////////////////////////////
// MARK: -
// MARK: Init/Dealloc
/////////////////////////////////////////////////////////////////////////////
...
- (void)commonInit
{
    ...
    __accessibilityElements = [[NSMutableDictionary dictionaryWithCapacity:0] retain];
...
-(void)dealloc
{
    [__accessibilityElements release], __accessibilityElements = nil;
...
-(void)addCustomLink:(NSURL*)linkUrl inRange:(NSRange)range {
    ...
    if ([customLinks count] == 1) {
        UIAccessibilityElement *linkAccessibilityElement = [[UIAccessibilityElement alloc] initWithAccessibilityContainer:self];
        linkAccessibilityElement.accessibilityLabel = [_attributedText.string substringWithRange:range];
        linkAccessibilityElement.accessibilityTraits = UIAccessibilityTraitLink;
        //TODO: Set Frame of accesibility element instead of the frame of the label
        linkAccessibilityElement.accessibilityFrame = self.frame;
        [__accessibilityElements setObject:linkAccessibilityElement forKey:link];
        [linkAccessibilityElement release];
    } else {
        [__accessibilityElements removeAllObjects];
    }
...
-(void)resetAttributedText {
	...
	CTLineBreakMode coreTextLBMode = CTLineBreakModeFromUILineBreakMode(UILineBreakModeCharacterWrap);
...
-(id)accessibilityElementAtIndex:(NSInteger)index {
    return [__accessibilityElements objectForKey:[customLinks objectAtIndex:index]];
}


-(NSInteger)indexOfAccessibilityElement:(id)element {
    
    NSTextCheckingResult *target = nil;
    
    for (id key in __accessibilityElements) {
        if ([__accessibilityElements objectForKey:key] == element) {
            target = key;
            break;
        }
            
    }
    return [customLinks indexOfObject:target];
}

-(BOOL)isAccessibilityElement {
    if ([__accessibilityElements count] > 0) {
        return NO;        
    } else {
        return YES;
    }
