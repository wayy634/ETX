...
- (void)dealloc {
    [_launcherView release] , _launcherView = nil;
    [_appControllers release] , _appControllers = nil;;
    [super dealloc];
}
...
//    NSLog(@"page = %@",self.launcherView.pages);
//    for (int i = 0; i < [self.launcherView.pages count]; i++) {
//        for (int j = 0; j < [(NSArray *)[self.launcherView.pages objectAtIndex:i] count]; j++) {
//            MyLauncherItem *temp = [[self.launcherView.pages objectAtIndex:i] objectAtIndex:j];
//            NSLog(@"temp = %@",temp.controllerTitle);
//        }
//    }
}
