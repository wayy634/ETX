...
/* iPhone */
static const int maxItemsPageCount = 30;
...
- (id)initWithFrame:(CGRect)frame 
{
    if ((self = [super initWithFrame:frame])) 
	{
		...
		[self setPagesScrollView:[[MyLauncherScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)]];
...
//        self.pagesScrollView.contentSize = CGSizeMake(self.frame.size.width, [[self.pages objectAtIndex:0] count]/4 * 80);
        self.pagesScrollView.contentSize = CGSizeMake(self.frame.size.width, 600);
        [self.pagesScrollView setBackgroundColor:[UIColor whiteColor]];
		[self addSubview:self.pagesScrollView];
        
//        NSLog(@"3=%f",self.pagesScrollView.frame.size.height);
...
- (void)setCurrentOrientation:(UIInterfaceOrientation)newOrientation {
    UIDeviceOrientation tmpNewOrientation = (UIDeviceOrientation)newOrientation;
    if (tmpNewOrientation != currentOrientation &&
        tmpNewOrientation != UIDeviceOrientationUnknown &&
        tmpNewOrientation != UIDeviceOrientationFaceUp &&
        tmpNewOrientation != UIDeviceOrientationFaceDown) {
        currentOrientation = tmpNewOrientation;
    }
}
...
//        [self resetOffY];
		minY = (K_OFFSET_Y * (((int)([(NSArray *)[self.pages objectAtIndex:0] count])/4) + 1)) + 20;
        if ([self.pages indexOfObject:page] == 0) {
            if (!line1) {
                line1 = [[UIView alloc] initWithFrame:CGRectMake(0, minY - 10, self.frame.size.width, 1)];
                [line1 setBackgroundColor:[LTools colorWithHexString:@"c1bab3"]];
                [self.pagesScrollView insertSubview:line1 atIndex:0];
            } else {
                [line1 setFrame:CGRectMake(0, minY - 10, self.frame.size.width, 1)];
            }
            
            if (!line2) {
                line2 = [[UIView alloc] initWithFrame:CGRectMake(0, minY - 9, self.frame.size.width, 1)];
                [line2 setBackgroundColor:[LTools colorWithHexString:@"ded9d2"]];
                [self.pagesScrollView insertSubview:line2 atIndex:0];
            } else {
                [line2 setFrame:CGRectMake(0, minY - 9, self.frame.size.width, 1)];
            }
            
            if (!contentView) {
                contentView = [[UIView alloc] initWithFrame:CGRectMake(0, minY - 8, self.frame.size.width, self.pagesScrollView.contentSize.height - (minY - 8))];
                [contentView setBackgroundColor:[LTools colorWithHexString:@"fff3d7"]];
                [self.pagesScrollView insertSubview:contentView atIndex:0];
            }else {
                [contentView setFrame:CGRectMake(0, minY - 8, self.frame.size.width, self.pagesScrollView.contentSize.height - (minY - 8))];
            }
        }
	}
    
    MyLauncherItem *temp = [[self.pages objectAtIndex:0] lastObject];
    if ([(NSArray *)[self.pages objectAtIndex:1] count] == 0) {
        [temp.backGroundImageV setImage:[UIImage imageNamed:@"icon_remove.png"]];
        [self makeToast:@"建议您减少自定义区域彩种" duration:2.0 position:@"custom"];
    } else {
        [temp.backGroundImageV setImage:[UIImage imageNamed:@"icon_defult.png"]];
    }
	
	self.pageControl.numberOfPages = self.pages.count;
//	self.pagesScrollView.contentSize = CGSizeMake(self.frame.size.width, [[self.pages objectAtIndex:0] count]/4 * 80);
//    self.pagesScrollView.contentSize = CGSizeMake(self.frame.size.width, 210 + [[self.pages objectAtIndex:1] count]/4 * 80);
	
	itemsAdded = YES;
}

- (void)resetPosition:(int)_index {
    [self setupCurrentViewLayoutSettings];
	for (NSMutableArray *page in self.pages) {
        CGFloat x = minX;
        CGFloat y = minY;
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if(dragging) {
		for (UITouch* touch in touches) {
            ...
            if (isFirst) {
                isIndex = page == 0 ? YES: NO;
                isFirst = NO;
                oldCount = [(NSArray *)[self.pages objectAtIndex:0] count];
            }
            if (sindex == oldCount-1 && page == 0 && isIndex) {
                return;
            }
            
			CGPoint location = [touch locationInView:self];
			self.draggingItem.center = CGPointMake(location.x + self.pagesScrollView.contentOffset.x, location.y + self.pagesScrollView.contentOffset.y);
            [self setupCurrentViewLayoutSettings];
            
            int currentPageIndex;
            if (location.y + self.pagesScrollView.contentOffset.y <= line1.frame.origin.y) {
                currentPageIndex = 0;
            } else {
                currentPageIndex = 1;
            }
//            NSInteger currentPageIndex = floor((location.y + self.pagesScrollView.contentOffset.y)/K_OFFSET_Y);
			...
			NSInteger dragItemRow = floor((dragItemY - (currentPageIndex * line1.frame.origin.y))/distanceHeight); // item height
			NSInteger dragIndex = (dragItemRow * columnCount) + dragItemColumn;
            
//            NSLog(@"==========");
//            NSLog(@"self.pagesScrollView.contentOffset.y = %f",self.pagesScrollView.contentOffset.y);
//            NSLog(@"dragIndex = %i",dragIndex);
//            NSLog(@"currentPageIndex = %i",currentPageIndex);
//            NSLog(@"page = %i",page);
//            NSLog(@"sindex = %i",sindex);
//            NSLog(@"isIndex = %i",isIndex);
//            NSLog(@"oldCount = %i",oldCount);
            BOOL isCountChange = NO;
...
//            if ((dragIndex < 3 && currentPageIndex == 0) ||
//                (currentPageIndex == 0 && [(NSArray *)[self.pages objectAtIndex:0] count] >= 8 && !isIndex && oldCount >=8)||
//                (dragIndex == oldCount-1 && currentPageIndex == 0)) {
//                return;
//            }
            
			if(sindex != dragIndex || currentPageIndex != page)
			{
                    ...
                    if ([self itemMovable:itemToMove]) {
                        ...
                        if(dragIndex >= currentPage.count) {
                            ...
                            if (currentPageIndex == 0) {
                                if (!self.draggingItem.isHaveCorner) {
                                    [self.draggingItem.closeButton setBackgroundImage:[UIImage imageNamed:@"homePage_custom_lottery_remove.png"] forState:UIControlStateNormal];
                                }
                                [currentPage insertObject:self.draggingItem atIndex:(dragIndex-1)];
                            } else {
                                if (!self.draggingItem.isHaveCorner) {
                                    [self.draggingItem.closeButton setBackgroundImage:[UIImage imageNamed:@"homePage_custom_lottery_add.png"] forState:UIControlStateNormal];
                                }
                                [currentPage insertObject:self.draggingItem atIndex:dragIndex];
                            }
//                            if ([(NSArray *)[self.pages objectAtIndex:0] count] < 8) {
//                                [((MyLauncherItem *)[[self.pages objectAtIndex:0] lastObject]) setHidden:NO];
                                [((MyLauncherItem *)[[self.pages objectAtIndex:0] lastObject]) layoutItem];
//                            } else {
//                                [((MyLauncherItem *)[[self.pages objectAtIndex:0] lastObject]) setHidden:YES];
//                            }
                            [self organizePages];
                            [UIView animateWithDuration:0.3
                                             animations:^{
                                                 [self resetPosition:currentPageIndex];
                                             }];
                        } else  {
                            if (isCountChange) {
                                dragIndex = currentPage.count-1;
                            }
                            [currentPage insertObject:self.draggingItem atIndex:dragIndex];
                            if (currentPageIndex == 0) {
                                if (!self.draggingItem.isHaveCorner) {
	...
	for (NSMutableArray *page in self.pages)
	{
		...
		for (MyLauncherItem *aitem in page)
		{
			if(aitem == ditem)
			{
                ...
                if (index == 0) {
                    for (int i = 0; i < [(NSArray *)[self.pages objectAtIndex:1] count]; i++) {
                        MyLauncherItem *tempItem = [[self.pages objectAtIndex:1] objectAtIndex:i];
                        if (!tempItem.isHaveCorner) {
                            targetIndex = i;
                            break;
                        }
                    }
                    [[self.pages objectAtIndex:1] insertObject:aitem atIndex:targetIndex];
                    if (!ditem.isHaveCorner) {
                        [aitem.closeButton setBackgroundImage:[UIImage imageNamed:@"homePage_custom_lottery_add.png"] forState:UIControlStateNormal];
                    }
...
//                if (index == 1 && [(NSArray *)[self.pages objectAtIndex:0] count] < 8) {
                    [[self.pages objectAtIndex:0] insertObject:aitem atIndex:([(NSArray *)[self.pages objectAtIndex:0] count] - 1)];
                    if (!ditem.isHaveCorner) {
                        [aitem.closeButton setBackgroundImage:[UIImage imageNamed:@"homePage_custom_lottery_remove.png"] forState:UIControlStateNormal];
                    }
                    [page removeObjectAtIndex:i];
                    [UIView animateWithDuration:0.3
                                     animations:^{
...
//                    }
                    return;
//                } else {
//                    [self makeToast:@"最多选择4个" duration:2.0 position:@"custom"];
//                    return;
//                }
