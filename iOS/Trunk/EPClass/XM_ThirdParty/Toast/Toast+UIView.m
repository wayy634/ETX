...
- (void)makeToastAtCustomPosition:(NSString *)message
{
    [self makeToast:message duration:2.f position:@"custom"];
}
...
- (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(id)point {
    ...
    if (![LTools getKIsShow]) {
        [LTools setKisShow:YES];
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position andPoint:(CGPoint) _point
{
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    toast.center = _point;
    toast.alpha = 0.0;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - (self.bounds.size.height / 4));
        }
