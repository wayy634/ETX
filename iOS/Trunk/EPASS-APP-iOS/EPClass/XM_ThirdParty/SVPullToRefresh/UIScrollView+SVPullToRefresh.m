...
//        [self addSubview:view.animationV];
        ...
        [view.finishImageV setFrame:CGRectMake((self.width - 34)/2, -35, 22, 22)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler customImage:(UIImage *)_image {
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        yOrigin = -SVPullToRefreshViewHeight;
        SVPullToRefreshView *view = [[SVPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight)];
        ...
        [backGroundImageV setFrame:CGRectMake(0, -ceil(scale*backGroundImage.size.height) - 40 + K_TITLE_OFFSET_Y, K_SCREEN_WIDTH, ceil(scale*backGroundImage.size.height))];
        [self addSubview:backGroundImageV];
        backGroundImage = nil;
        
        view.customArrow = [[[UIImageView alloc] initWithImage:_image] autorelease];
        [view.customArrow setFrame:CGRectMake(120, -40 + K_TITLE_OFFSET_Y, _image.size.width, _image.size.height)];
        [self addSubview:view.customArrow];
        [self setBackgroundColor:[LTools colorWithHexString:@"ebebeb"]];
    }
}

- (void)triggerPullToRefreshInMission {
    self.pullToRefreshView.state = SVPullToRefreshStateTriggered;
    self.pullToRefreshView.mOtherHeight = -175;
...
@synthesize mOtherHeight = _mOtherHeight;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.textColor = [LTools colorWithHexString:@"9296a3"];
...
//                        [self.animationV animation:LCAnimationImageVAnimationTypeOn];
        ...
        if(subtitleSize.height > 0){
            ...
            CGFloat titleY = minY;// + (self.customArrow ? K_TITLE_OFFSET_Y : 0);
            ...
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, titleSize.width, titleSize.height));
            if (self.customArrow) {
                [self.customArrow setFrame:CGRectMake(self.titleLabel.left - self.customArrow.width - 8, self.customArrow.top, self.customArrow.width, self.customArrow.height)];
            }
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY + titleSize.height + marginY, subtitleSize.width, subtitleSize.height));
        }
...
- (void)setShadowView:(UIImage *)_shadowImage backGroundColor:(UIColor *)_color radius:(float)_radius; {
    UIView *backGroundV = [[UIView alloc] initWithFrame:CGRectMake(_radius, -400, self.width - _radius*2, self.height+400)];
    ...
    [leftShadow setFrame:CGRectMake(-_radius, 0, _radius, backGroundV.height)];
    [rightShadow setFrame:CGRectMake(backGroundV.width, 0, _radius, backGroundV.height)];
...
//                NSLog(@"mOtherHeight : %d",self.mOtherHeight);
                [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentOffset.x, -self.frame.size.height+self.mOtherHeight)
                                         animated:NO];
            }completion:^(BOOL finished){
...
- (void)setMOtherHeight:(int)mOtherHeight
{
    if (_mOtherHeight == mOtherHeight)
        return;
    _mOtherHeight = mOtherHeight;
}
